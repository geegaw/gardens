define(function(){
	var GardenToolkit = {
		...
		formToAssoc: function($form){
			var assoc = {};
		    $form.find('input:not(:submit):not(.ignore):enabled, select:not(.ignore):enabled, textarea:not(.ignore):enabled').each(function(){
		...
		assocToForm: function($form, assoc){
		...
		slugify: function(s){
			s = s.replace( /[^a-zA-Z0-8_-]/, '' );
			s = s.replace( '-', '_' );
			return s.toLowerCase();
		}
	};
	return GardenToolkit;
});